...
import { fetchCursorPaginatedItems } from "./items-api";

export function usePaginatedItems(): UsePaginatedItemsResult {
  ...
  const fetchPage = useCallback(
    async (size: PageSize, cursor?: string): Promise<void> => {
      ...
      try {
        const data = await fetchCursorPaginatedItems(
        ...
        if (cursor === undefined) {
          ...
          setPageCache((prev) => [...prev, newPage]);
  ...
  const items: Item[] = currentPage?.items ?? [];
