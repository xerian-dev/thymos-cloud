...
import type {
  ...
  CachedPage,
  PageSize,
  UseCursorPaginatedAccountsResult,
} from "./accounts-types";
import { fetchCursorPaginatedAccounts } from "./accounts-api";

export function usePaginatedAccounts(): UseCursorPaginatedAccountsResult {
  ...
  const fetchPage = useCallback(
    async (size: PageSize, cursor?: string): Promise<void> => {
      ...
      try {
        const data = await fetchCursorPaginatedAccounts(
        ...
        if (cursor === undefined) {
          ...
          setPageCache((prev) => {
            ...
            // Use setTimeout to ensure the index update happens after the cache update
            // This avoids reading stale cache length from the closure
            return updated;
          });
  ...
  const accounts: Account[] = currentPage?.accounts ?? [];
